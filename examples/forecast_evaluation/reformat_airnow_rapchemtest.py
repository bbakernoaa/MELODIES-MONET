...
dfp = df.rename({"siteid": "x"}, axis=1).pivot_table(
...
dfpsite = df.rename({"siteid": "x"}, axis=1).drop_duplicates(
    subset=["x"]
)  # dropping duplicates and renaming
# convert sites to xarray
test = (
    dfpsite.drop(["time", "time_local", "variable", "obs"], axis=1)
    .set_index("x")
...
t = (
    tt.expand_dims("y")
    .set_coords(["siteid", "latitude", "longitude"])
    .transpose("time", "y", "x")
)
