...
class analysis:
    ...
    def _migrate_control_dict(self):
        ...
        new_dict = {
            ...
            "models": self.control_dict.get(
                "models", self.control_dict.get("model", {})
            ).copy(),
        ...
        if "gridded_pairing" in self.control_dict:
            for pair_label, pair_cfg in self.control_dict["gridded_pairing"].items():
                if pair_label not in new_dict["evaluations"]:
                    ...
                    if "obs" in pair_cfg:
                        new_dict["evaluations"][pair_label]["reference"] = pair_cfg[
                            "obs"
                        ]
                    if "model" in pair_cfg:
                        new_dict["evaluations"][pair_label]["test_models"] = [
                            pair_cfg["model"]
                        ]
        ...
        if "stats" in self.control_dict:
            ...
            for p_label in pair_labels:
                if p_label in new_dict["evaluations"]:
                    ...
                    new_dict["evaluations"][p_label]["stats"]["_group_id"] = (
                        stats_block_id
                    )
                else:
        ...
        if "models" in self.control_dict:
            ...
            for mod in self.control_dict["models"]:
                ...
                if "radius_of_influence" in self.control_dict["models"][mod].keys():
                    mod_inst.radius_of_influence = self.control_dict["models"][mod][
                        "radius_of_influence"
                    ]
                else:
                ...
                if isinstance(self.control_dict["models"][mod]["files"], list):
                    mod_inst.file_str = [
                        ...
                        for f in self.control_dict["models"][mod]["files"]
                    ]
                else:
                    mod_inst.file_str = os.path.expandvars(
                        self.control_dict["models"][mod]["files"]
                    )
                ...
                if "variables" in self.control_dict["models"][mod].keys():
                    mod_inst.variable_dict = self.control_dict["models"][mod][
                        "variables"
                    ]
                if "variable_summing" in self.control_dict["models"][mod].keys():
                    mod_inst.variable_summing = self.control_dict["models"][mod][
                        "variable_summing"
                    ]
                if "plot_kwargs" in self.control_dict["models"][mod].keys():
                    mod_inst.plot_kwargs = self.control_dict["models"][mod][
                        "plot_kwargs"
                    ]
                ...
                if mod_inst.model in ["cesm_se"]:
                    if "scrip_file" in self.control_dict["models"][mod].keys():
                        mod_inst.scrip_file = self.control_dict["models"][mod][
                            "scrip_file"
                        ]
                    else:
                        raise ValueError(
            ...
            for obs_time in self.obs[obs].obj:
                ...
                for var in self.obs[obs].obj[obs_time]:
                    ...
                    n_obs = self.obs[obs].obj[obs_time][var].size
                            # NOTE: This part still uses .values for grid updates because update_data_grid
                            # likely expects numpy arrays for low-level processing.
                            # We keep it as is for now but acknowledge it breaks laziness.
        ...
        for eval_label, eval_cfg in self.control_dict["evaluations"].items():
            ...
            for mod_label in test_models:
                ...
                if ref_label in self.obs:
                    ...
                    print(
                        f"Warning: Model-to-Model pairing ({ref_label} vs {mod_label}) is partially implemented."
                    )
                ...
                if obs_type == "pt_sfc":
                    ...
                    # now combine obs with model using xarray-native pairing to maintain laziness
                    paired_obj = model_obj.monet.combine_point(
                        obs.obj,
                    ...
                    if self.debug:
                        print("After pairing: ", paired_obj)
                ...
                # if aircraft (aircraft observation)
                    ...
                    # Maintain xarray/dask throughout aircraft pairing
                    obs_obj = obs.obj
                    if hasattr(obs_obj, "dropna"):
                         obs_obj = obs_obj.dropna(dim="time", subset=["pressure_obs", "latitude", "longitude"])

                    # Use monet's combinetool for spatial alignment
                    # Note: This assumes obs_obj has 'latitude' and 'longitude'
                    ds_model = m.util.combinetool.combine_da_to_da(
                        model_obj, obs_obj, merge=False
                    )

                    # Interpolate based on time in the observations
                    # Use squeeze only if time dimension exists and is 1
                    time_obs = ds_model.time.load() if "time" in ds_model.coords else ds_model.coords["time"]
                    ds_model = ds_model.interp(time=time_obs)

                    # Vertical interpolation using refactored vert_interp (now xarray-native)
                    paired_obj = vert_interp(ds_model, obs_obj, keys + mod_vars)
                    ...
                    for grp_sonde, grp_dict_sonde in plot_dict_sonde.items():
                        ...
                        if plot_type_sonde in plot_sonde_type_list_all:
                            ...
                            obs.obj = obs.obj.loc[
                                obs.obj["station"] == station_name_sonde[0]
                            ]
                            obs.obj = obs.obj.loc[
                                datetime.datetime(
                                    cds_sonde[0],
                ...
                # if sat_swath_clm (satellite l2 column products)
                    ...
                    if (
                        "apply_ak" not in self.pairing_kwargs.get(obs_type, {})
                        and "apply_ak" not in methods
                        and "no_ak" not in methods
                    ):
                        print(
                    ...
                    if obs.sat_type == "omps_nm":
                        ...
                        paired_data = paired_data.where(
                            paired_data.ozone_column.notnull()
                        )
                        p = pair()
                        p.type = obs.obs_type
                        p.obs = obs.label
                    ...
                    if obs.sat_type == "tropomi_l2_no2":
                        ...
                        if pairing_kws["mod_to_overpass"]:
                            ...
                            print(
                                "Warning: The pairing_kwarg mod_to_overpass is False."
                            )
                            print(
                    ...
                    if "tempo_l2" in obs.sat_type:
                        ...
                        regrid_method = (
                            obs.regrid_method
                            if obs.regrid_method is not None
                            else "bilinear"
                        )
    ...
    def plotting(self):
        ...
        for grp, grp_dict in plot_dict.items():
            ...
            for obsvar in obs_vars:
                ...
                pair_labels_obsvar = [
                    p for p in pair_labels if obsvar in self.paired[p].obs_vars
                ]
                for domain in range(len(domain_types)):
                    ...
                    for p_index, p_label in enumerate(pair_labels_obsvar):
                        ...
                        if domain_type != "all":
                            p_region = select_region(
                                p.obj, domain_type, domain_name, domain_info
                            )
                        else:
                            p_region = p.obj
                        ...
                        if obs_type in [
                            ...
                            if (
                                "time" not in p_region.dims
                                and obs_type == "sat_swath_clm"
                            ):
                        ...
                        if (
                            p.obs in self.obs
                            and self.obs[p.obs].variable_dict is not None
                        ):
                        ...
                        if obs_type in [
                            ...
                            pairdf = pairdf_all.where(
                                pairdf_all[obsvar].notnull()
                                & pairdf_all[modvar].notnull()
                            )
                        ...
                        # Types of plots
                        if (
                            plot_type.lower() == "timeseries"
                            or plot_type.lower() == "diurnal"
                        ):
                            ...
                            # Use xarray's set_index if it's a dataset, otherwise pandas
                            if isinstance(pairdf, xr.Dataset):
                                if grp_dict["data_proc"]["ts_select_time"] in pairdf.coords or grp_dict["data_proc"]["ts_select_time"] in pairdf.data_vars:
                                     # For xarray we might want to ensure 'time' dimension is correct
                                     pass
                            elif isinstance(pairdf, pd.core.frame.DataFrame):
                        ...
                        # qzr++ Added vertprofile plotype for aircraft vs model comparisons
                            ...
                            try:
                                ...
                                if p_label in self.control_dict["evaluations"]:
                                    _ = self.control_dict["evaluations"][p_label][
                                        "mapping"
                                    ]
                                else:
                                    # Fallback for unexpected cases
                                    _ = self.control_dict["models"][model_label][
                                        "mapping"
                                    ][obs_label]
                            except KeyError:
                                print(
                            ...
                            if set_yaxis is True:
                                ...
                                plot_kwargs["ty_scale"] = 1.5  # Use default
                            try:
                                plot_kwargs["ty_scale"] = grp_dict["data_proc"].get(
                                    "ty_scale", 1.5
                                )
                            except KeyError:
                            ...
                            plot_kwargs = {
                                ...
                                **{
                                    ...
                                    "nlevels": grp_dict["data_proc"].get(
                                        "nlevels", None
                                    ),
                                    "fig_dict": fig_dict,
                                    "text_dict": text_dict,
    ...
    def stats(self):
        ...
        evals_with_stats = [
            e
            for e in self.control_dict["evaluations"]
            if "stats" in self.control_dict["evaluations"][e]
        ]
        ...
        for group_id, group_info in stat_groups.items():
            ...
            stat_fullname_s = proc_stats.produce_stat_dict(
                stat_list=stat_list, spaces=True
            )
                ...
                for domain in range(len(domain_types)):
                    ...
                    for p_label in pair_labels:
                        ...
                        if self.obs[p.obs].variable_dict is not None:
                            obs_plot_dict = self.obs[p.obs].variable_dict.get(
                                obsvar, {}
                            )
                        else:
                            obs_plot_dict = {}
                        ...
                        if not final_title:
                            if cal_reg:
                                ylabel = obs_plot_dict.get(
                                    "ylabel_reg_plot", f"{obsvar}_reg"
                                )
                            else:
                        ...
                        dim_order = [
                            dim for dim in ["time", "y", "x"] if dim in p_region.dims
                        ]
                        ...
                        if cal_reg:
                            ...
                            if obsvar == "PM2.5":
                                pairdf_reg = splots.make_24hr_regulatory(
                                    ...
                                    columns={
                                        f"{obsvar}_y": f"{obsvar}_reg",
                                        f"{modvar}_y": f"{modvar}_reg",
                                    }
                                )
                            elif obsvar == "OZONE":
                                pairdf_reg = splots.make_8hr_regulatory(
                            ...
                            pairdf_stats = pairdf_reg.reset_index().dropna(
                                subset=[f"{modvar}_reg", f"{obsvar}_reg"]
                            )
                    ...
                    out_path = (
                        os.path.join(self.output_dir, outname)
                        if self.output_dir
                        else outname
                    )
                    ...
                    if stat_dict.get("output_table", False):
                        ...
                        proc_stats.create_table(
                            ...
                            outname=out_path,
                            title=final_title,
                            out_table_kwargs=stat_dict.get("output_table_kwargs", {}),
                            debug=self.debug,
                        )
