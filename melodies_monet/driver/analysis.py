    ...
    def pair_data(self, time_interval=None):
        ...
        for model_label in self.models:
            ...
            for obs_to_pair in mod.mapping.keys():
                ...
                # Leverage new upstream monet.pair functionality via MM pair class
                p = pair()
                p.obs = obs.label
                p.model = mod.label
                p.model_vars = keys
                ...
                # Use the new pair function in monet
                # This automatically handles pt_sfc, aircraft, sonde, mobile, and ground
                p.obj = p.pair_data(model_obj, obs.obj, radius_of_influence=mod.radius_of_influence,
                                   suffix=mod.label, interp_time=True)
