...
class analysis:
    ...
    def open_models(self, time_interval=None,load_files=True):
        ...
        if 'model' in self.control_dict:
            ...
            for mod in self.control_dict['model']:
                config = self.control_dict['model'][mod]
                m = factory.create_model(mod, config, time_interval=time_interval, load_files=load_files, control_dict=self.control_dict)
    ...
    def open_obs(self, time_interval=None, load_files=True):
        ...
        if 'obs' in self.control_dict:
            ...
            for obs in self.control_dict['obs']:
                config = self.control_dict['obs'][obs]
                o = factory.create_observation(obs, config, time_interval=time_interval, load_files=load_files, control_dict=self.control_dict)
                self.obs[o.label] = o
    ...
    def setup_obs_grid(self):
        ...
        """
        gridder = ObsGridder(self)
        gridder.setup_obs_grid()
    ...
    def update_obs_gridded_data(self):
        ...
        """
        gridder = ObsGridder(self)
        gridder.update_obs_gridded_data()
    ...
    def normalize_obs_gridded_data(self):
        ...
        """
        gridder = ObsGridder(self)
        gridder.normalize_obs_gridded_data()
