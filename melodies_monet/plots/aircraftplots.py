...
def make_curtain_plot(
    ...
    fig, axs = plt.subplots(
        nrows=2, figsize=grp_dict.get("fig_kwargs", {}).get("figsize", (20, 8)), sharex=True, gridspec_kw={"height_ratios": [1, 1]}
    )
    ...
    axs[0].set_ylabel(y_axis_label, fontsize=text_dict.get("fontsize", 18), fontweight=text_dict.get("fontweight", "bold"))
    axs[1].set_ylabel(y_axis_label, fontsize=text_dict.get("fontsize", 18), fontweight=text_dict.get("fontweight", "bold"))
    ...
    _ = axs[1].scatter(
        time_dates, obs_pressure, c=pairdf[obs_var].values, cmap=cmap, norm=norm, edgecolor=(0, 0, 0, 0), linewidth=0.3, alpha=0.5
    )
    ...
    axs[0].scatter(
        time_dates, obs_pressure, c=pairdf[mod_var].values, cmap=cmap, norm=norm, edgecolor="black", linewidth=0.5, alpha=0.7
    )
