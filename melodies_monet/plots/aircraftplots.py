...
def add_yax2_altitude(ax, pairdf, altitude_yax2, text_kwargs, vmin_y2, vmax_y2):
    ...
    ax2.yaxis.set_ticks(
        np.arange(start_tick, vmax_y2 + altitude_ticks + 1, altitude_ticks)
    )
...
def make_curtain_plot(
    ...
    print(
        f"cmin and cmax set dynamically based on model and observation data to {cmin} and {cmax}"
    )
    ...
    if (
        "color_map_custom" in grp_dict
        and grp_dict["color_map_custom"] is True
        and "colors" in grp_dict
    ):
        # Custom color map logic
        colors = grp_dict["colors"]
        cmap = LinearSegmentedColormap.from_list(
            "custom_cmap", colors, N=grp_dict.get("color_levels", 100)
        )
    else:
    ...
    axs[
        1
    ].invert_yaxis()  # Invert y-axis to have max pressure at the bottom (FOR SECOND SUBPLOT)
...
def make_vertprofile(
    ...
    if ax is None:
        ...
        q1 = df.groupby(altitude_bins, observed=True)[column].quantile(
            0.25, numeric_only=True
        )
        q3 = df.groupby(altitude_bins, observed=True)[column].quantile(
            0.75, numeric_only=True
        )
        ...
        p10 = df.groupby(altitude_bins, observed=True)[column].quantile(
            0.10, numeric_only=True
        )
        p90 = df.groupby(altitude_bins, observed=True)[column].quantile(
            0.90, numeric_only=True
        )
        ...
        plot_kwargs_fillbetween["label"] = (
            f"{label} (interquartile 25th-75th percentile range)"
        )
        ...
        median_line_df = pd.DataFrame(
            data={"median": median.values, "binmidpoint": binmidpoint.values}
        )
        ax = median_line_df.plot(
            x="median", y="binmidpoint", ax=ax, legend=True, **plot_kwargs
        )
    ...
    # If plot has been created, add to the current axes
        ...
        ax = median_line_df.plot(
            x="median", y="binmidpoint", ax=ax, legend=True, **plot_dict
        )
...
def make_scatter_density_plot(
    ...
    **kwargs: dict
    ...
    if extensions:
        cbar = plt.colorbar(
            mappable, extend="both", ax=ax
        )  # Extends the colorbar at both ends
    else:
...
def calculate_violin(
    df, column=None, label=None, plot_dict=None, comb_violin=None, label_violin=None
):
    """
...
def make_violin_plot(
    ...
    plt.ylabel(
        ylabel if ylabel else "Value", weight="bold", fontsize=text_kwargs["fontsize"]
    )
