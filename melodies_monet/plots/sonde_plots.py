...
def make_vertical_single_date(
    df, comb_bx, altitude_range, altitude_method, vmin, vmax, station_name, release_time, label_bx, fig_dict, text_dict
):
    ALT_sl = df["altitude"]
...
def density_scatter_plot_os(df, altitude_range, vmin, vmax, station_name, altitude_method, cmap_method, modvar, obsvar):
    ...
    sc = plt.scatter(
        O3_OBS, O3_MODEL, c=ALT, vmin=altitude_range[0], vmax=altitude_range[1], cmap="jet", edgecolors="k", linewidth=0.5, s=15
    )
    cb = plt.colorbar(sc)
    if altitude_method[0] == "ground level":
