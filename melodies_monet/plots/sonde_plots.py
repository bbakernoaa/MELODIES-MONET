...
def make_vertical_single_date(
    ...
    if altitude_method[0] == "ground level":
        height_value = df_height.loc[df_height["station"] == station_name[0]].values[0][
            1
        ]
    ...
    plt.title(
        "Comparison at " + str(station_name[0]) + "\non " + str(release_time) + " UTC",
        **text_kwargs,
    )
    plt.legend()
...
def make_vertical_boxplot_os(
    ...
    position_list_mid = [
        (position_list_1[i] + position_list_2[i]) / 2
        for i in range(len(position_list_1))
    ]
    ...
    for i in range(len(O3_MODEL_ALL)):
        output_list_model = split_by_threshold(
            O3_MODEL_ALL[i], alt_p, altitude_threshold_list
        )
        _ = ax.boxplot(
            output_list_model,
            vert=False,
...
def split_by_threshold(o3_list_input, alt_list_input, threshold_list_input):
    ...
    for i in range(1, len(threshold_list_input)):
        df_here = df.o3.loc[
            (df.alt > threshold_list_input[i - 1]) & (df.alt <= threshold_list_input[i])
        ]
        output_list.append(df_here.values)
    return output_list


def density_scatter_plot_os(
    df,
    altitude_range,
    vmin,
    vmax,
    ...
    sc = plt.scatter(
        ...
        O3_MODEL,
        c=ALT,
        vmin=altitude_range[0],
        vmax=altitude_range[1],
        cmap="jet",
        edgecolors="k",
        linewidth=0.5,
        s=15,
