...
def map_projection(m, *, model_name=None):
    ...
    if mod == "cmaq":
        proj = ccrs.LambertConformal(central_longitude=m.obj.XCENT, central_latitude=m.obj.YCENT)
    elif mod in {"wrfchem", "rapchem"}:
        if m.obj.MAP_PROJ == 1:
            proj = ccrs.LambertConformal(central_longitude=m.obj.CEN_LON, central_latitude=m.obj.CEN_LAT)
    ...
    # Need to add the projections you want to use for the other models here.
        ...
        print(f"NOTE: Projection not defined for model {mod!r}. " "Please add to surfplots.py. " "Setting to `ccrs.PlateCarree()`.")
        proj = ccrs.PlateCarree()
    return proj
...
def make_timeseries(
    ...
    ax.legend(frameon=False, fontsize=text_kwargs["fontsize"] * 0.8, bbox_to_anchor=(1.0, 0.9), loc="center left")
...
def make_diurnal_cycle(
    ...
    **kwargs,
    ...
    if ax is None:
        ...
        plot_kwargs = {**dict(linestyle="-", marker="*", linewidth=1.2, markersize=6.0), **plot_dict}
    time = df.index
    ...
    if shading_range is not None:
        if shading_range not in ["IQR", "std", "total_range"]:
            raise ValueError(f"shading_range = {shading_range}. IQR, std or total_range expected.")
...
def make_taylor(
    ...
    if dia is None:
        ...
        # plot the line
        if df_reg is not None:
            dia = td(df_reg[column_o + "_reg"].std(), scale=ty_scale, fig=f, rect=111, label=label_o)
    ...
    plt.grid(alpha=0.5)
    plt.legend(frameon=False, fontsize=text_kwargs["fontsize"] * 0.8, bbox_to_anchor=(0.75, 0.93), loc="center left")
    if domain_type is not None and domain_name is not None:
        if domain_type == "epa_region":
...
def calculate_multi_boxplot(
    df, df_reg=None, region_name=None, column=None, label=None, plot_dict=None, comb_bx=None, label_bx=None
):
    """Combines data into acceptable format for box-plot
...
def make_boxplot(
    ...
    boxplot_kwargs = {
        ...
        "order": order_box,
        "showmeans": True,
        "meanprops": {"marker": ".", "markerfacecolor": "black", "markeredgecolor": "black", "markersize": 20.0},
    }
...
def make_multi_boxplot(
    ...
    acro = region_list
    sns.boxplot(
        x="Regions", y="Value", hue="model", data=tdf.loc[tdf.Regions.isin(acro)], order=acro, showfliers=False, **boxplot_kwargs
    )
    ax.set_xlabel("")
    ax.set_ylabel(ylabel, fontweight="bold", **text_kwargs)
    ax.tick_params(labelsize=text_kwargs["fontsize"] * 0.8)
...
def scorecard_step1_combine_df(
    df, df_reg=None, region_name=None, urban_rural_name=None, column=None, label=None, plot_dict=None, comb_bx=None, label_bx=None
):
    """Combines data into acceptable format for box-plot
...
def scorecard_step9_makeplot(
    ...
    cb.ax.set_yticklabels(
        ["99.9% Worse", "99% Worse", "95% Worse", "No Significant Difference", "95% Better", "99% Better", "99.9% Better"]
    )
...
def make_spatial_bias_exceedance(
    ...
    if column_o == "OZONE_reg":
        ...
        df_counto = df[df[column_o] > 70.0].groupby(["siteid"], as_index=False)[column_o].count()
        df_countm = df[df[column_m] > 70.0].groupby(["siteid"], as_index=False)[column_m].count()
        ylabel2 = "O3"
        ...
        # get the exceedance days for each site
        df_counto = df[df[column_o] > 35.0].groupby(["siteid"], as_index=False)[column_o].count()
        df_countm = df[df[column_m] > 35.0].groupby(["siteid"], as_index=False)[column_m].count()
        ylabel2 = "PM2.5"
