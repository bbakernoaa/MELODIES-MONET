...
def map_projection(m, *, model_name=None):
    ...
    if mod == "cmaq":
        ...
        if m.obj.MAP_PROJ == 1:
            ...
            proj = ccrs.PlateCarree(central_longitude=m.obj.CEN_LON)
        else:
...
def make_spatial_bias(
    ...
    if df_reg is not None:
        ...
        if ptile is None:
            ...
            df_mean = df_reg.groupby(["siteid"], as_index=False).quantile(
                ptile / 100.0, numeric_only=True
            )
        ...
        if ptile is None:
            ...
            df_mean = df.groupby(["siteid"], as_index=False).quantile(
                ptile / 100.0, numeric_only=True
            )
...
def make_timeseries(
    ...
    if ax is None:
        ...
        if df_reg is not None:
            ax = (
                df_reg.set_index("time_local")[column + "_reg"]
                .resample("D")
                .mean()
                .plot(ax=ax, legend=True, **plot_kwargs)
            )
        else:
    ...
    # If plot has been created add to the current axes.
        ...
        if df_reg is not None:
            ax = (
                ...
                .resample("D")
                .mean()
                .plot(ax=ax, legend=True, **plot_dict)
            )
        else:
            if avg_window is None:
...
def make_diurnal_cycle(
    ...
    **kwargs,
    ...
    if ax is None:
        ...
        plot_kwargs = {
            **dict(linestyle="-", marker="*", linewidth=1.2, markersize=6.0),
            **plot_dict,
        }
    time = df.index
...
def make_spatial_overlay(
    ...
    if vmodel.attrs.get("mio_has_unstructured_grid", False):
        ...
        _ = Plot_2D(
            vmodel_mean,
            scrip_file=vmodel.mio_scrip_file,
            cmap=cmap,  # colorticks=clevel, colorlabels=clevel,
            cmin=vmin,
            cmax=vmax,
            lon_range=[lonmin, lonmax],
            lat_range=[latmin, latmax],
            ax=ax,
            state=fig_dict["states"],
        )
    else:
        # I add extend='both' here because the colorbar is setup to plot the values outside the range
        ax = vmodel_mean.monet.quick_contourf(
            cbar_kwargs=cbar_kwargs,
            figsize=map_kwargs["figsize"],
            map_kws=map_kwargs,
            robust=True,
            norm=norm,
            cmap=cmap,
    ...
    plt.tight_layout(pad=0)
    plt.title(
        title_add + label_o + " overlaid on " + label_m,
        fontweight="bold",
        **text_kwargs,
    )

    ax.axes.scatter(
        df_mean.longitude.values,
        df_mean.latitude.values,
        s=30,
        c=df_mean[column_o],
...
def calculate_multi_boxplot(
    ...
    if df_reg is not None:
        ...
        df_reg_epa = df_reg.merge(
            df_short[["siteid", "epa_region"]], how="left", on="siteid"
        )
...
def scorecard_step1_combine_df(
    df,
    df_reg=None,
    region_name=None,
...
def scorecard_step2_prepare_individual_df(
    comb_bx, region_bx, msa_bx, time_bx, model_name_list=None
):
    ...
    data_model1 = (
        comb_bx[comb_bx.columns[1]]
        .to_frame()
        .rename({comb_bx.columns[1]: "Value"}, axis=1)
    )
    data_model2 = (
        comb_bx[comb_bx.columns[2]]
        .to_frame()
        .rename({comb_bx.columns[2]: "Value"}, axis=1)
    )
...
def scorecard_step4_GetRegionLUCDate(
    ds_name=None, region_list=None, datelist=None, urban_rural_differentiate_value=None
):
    ...
    for region in region_list:  # region == 'R1'
        ...
        rural_index_list_here = scorecard_step3_getLUC(
            region, ds_name, urban_rural_differentiate_value
        )[0]
        ...
        for date in datelist:
            ...
            for i in range(len(region_here["Time"])):
                ...
                timestamp = (
                    date_here1 - np.datetime64("1970-01-01T00:00:00")
                ) / np.timedelta64(1, "s")
                date_here = datetime.utcfromtimestamp(
                    timestamp
                )  # this function== 1970,1,1 + timestamp(in seconds)
            ...
            region_date_rural_here_array = np.array(region_date_rural_here).reshape(
                (1, len(region_date_rural_here))
            )
            region_date_urban_here_array = np.array(region_date_urban_here).reshape(
                (1, len(region_date_urban_here))
            )
...
def CalcIOA(v2, v1):  # v1 is observation, v2 is prediction
    ...
    ioa_det = np.sum(
        (np.abs(np.subtract(v2, np.mean(v1))) + np.abs(np.subtract(v1, np.mean(v1))))
        ** 2
    )
...
def scorecard_step6_BetterOrWorse(
    obs_input=None, model1_input=None, model2_input=None, better_or_worse_method=None
):
...
def scorecard_step7_SigLevel(model1_input=None, model2_input=None):
    ...
    if (upper_bd_X1_95 - lower_bd_X2_95) * (upper_bd_X2_95 - lower_bd_X1_95) >= 0:
        ...
        if (upper_bd_X1_99 - lower_bd_X2_99) * (upper_bd_X2_99 - lower_bd_X1_99) >= 0:
            ...
            if (upper_bd_X1_999 - lower_bd_X2_999) * (
                upper_bd_X2_999 - lower_bd_X1_999
            ) >= 0:
...
def scorecard_step8_OutputMatrix(
    obs_urban_input=None,
    model1_urban_input=None,
    model2_urban_input=None,
    obs_rural_input=None,
    model1_rural_input=None,
    model2_rural_input=None,
    better_or_worse_method=None,
):

    output_matrix = np.zeros(
        (len(obs_urban_input) + len(obs_rural_input), len(obs_urban_input[0]))
    )
    for i in range(len(obs_urban_input) + len(obs_rural_input)):  # i is region
        for j in range(len(obs_urban_input[0])):  # j is date
            ...
            if OBS_here == []:
                ...
                key_word_BW = scorecard_step6_BetterOrWorse(
                    OBS_here, model1_here, model2_here, better_or_worse_method
                )  # ORDER: airnow, cmaq_54, cmaq_52
                key_word_SL = scorecard_step7_SigLevel(
                    model1_here, model2_here
                )  # ORDER: cmaq_54, cmaq_52
...
def scorecard_step9_makeplot(
    ...
    plot1 = plt.pcolormesh(
        output_matrix, cmap="coolwarm", edgecolor="k", vmin=-100, vmax=100
    )
...
def Plot_CSI(
    ...
    for i in range(1, len_model):
        ...
        for j in range(len(threshold_list)):
            csi_here = Calc_Score(
                score_name_input, threshold_list[j], model_input, obs_input
            )
    ...
    if domain_type is not None and domain_name is not None:
        if domain_type == "epa_region":
            ax.set_title(
                column + " EPA Region " + domain_name, fontweight="bold", **text_kwargs
            )
        else:
            ax.set_title(column + " " + domain_name, fontweight="bold", **text_kwargs)


def make_spatial_bias_exceedance(
    df,
    column_o=None,
    label_o=None,
    column_m=None,
    ...
    if column_o == "OZONE_reg":
        df_mean = df.groupby(["siteid"], as_index=False).quantile(
            0.95, numeric_only=True
        )  # concentrations not used in plotting, get the correct format for plotting
        # get the exceedance days for each site
        df_counto = (
            df[df[column_o] > 70.0]
            .groupby(["siteid"], as_index=False)[column_o]
            .count()
        )
        df_countm = (
            df[df[column_m] > 70.0]
            .groupby(["siteid"], as_index=False)[column_m]
            .count()
        )
        ylabel2 = "O3"
    ...
    # combine dataframes
    df_combine = (
        df_counto.set_index(["siteid"])
        .join(df_countm.set_index(["siteid"]), on=(["siteid"]), how="outer")
        .reset_index()
    )
    ...
    df_reg = (
        ...
        .rename(
            index=str,
            columns={
                column_o + "_y": column_o + "_day",
                column_m + "_y": column_m + "_day",
            },
        )
