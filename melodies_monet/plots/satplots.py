...
def map_projection(f):
    ...
    if f.model.lower() == "cmaq":
        ...
        if f.obj.MAP_PROJ == 1:
            proj = ccrs.LambertConformal(central_longitude=f.obj.CEN_LON, central_latitude=f.obj.CEN_LAT)
        elif f.MAP_PROJ == 6:
...
def make_timeseries(
    ...
    if ax is None:
        ...
        if avg_window is None:
            ...
            # bug fixed (AttributeError: 'Rectangle' object has no property 'marker'). M.Li
            df[column].resample(time=avg_window).mean().mean("y").plot.line(
                x="time",
                ax=ax,
                color=plot_kwargs["color"],
...
def make_taylor(
    ...
    if dia is None:
        ...
        sns.set_style("ticks")
        # plot the line
        dia = td(df[column_o].std().values, scale=ty_scale, fig=f, rect=111, label=label_o)
