...
class Plot_2D(object):
    def __init__(
        ...
        if type(var) in [xr.core.dataset.Dataset, xr.core.dataarray.DataArray]:
            if np.ndim(var) == 2:  # FV results
                ...
                if lons is not None:
                    print(
                        'Warning: "lons" is assigned but not used '
                        + "because xarray itself has longitude values"
                    )
                if lats is not None:
                    print(
                        'Warning: "lats" is assigned but not used '
                        + "because xarray itself has longitude values"
                    )
        ...
        if center_180:
            if projection == ccrs.PlateCarree():
                ...
                if projection.proj4_params["lon_0"] != 180:
                    raise ValueError(
                        "central_longitude must be set to 180 if center_180 is True"
                    )
        ...
        if self.model_type == "SE":
            self.lons_corners = np.copy(
                self.corner_lon.reshape(
                    self.corner_lon.shape[0], self.corner_lon.shape[1], 1
                )
            )
            self.lats_corners = np.copy(
                self.corner_lat.reshape(
                    self.corner_lat.shape[0], self.corner_lat.shape[1], 1
                )
            )
            ...
            for i, cenlon in enumerate(self.center_lon):
                lon_maxmin = np.max(self.lons_corners[i, :, :]) - np.min(
                    self.lons_corners[i, :, :]
                )
                if self.center_180:
                    if lon_maxmin > 180:
            ...
            if self.lons_corners_add != []:
                self.lons_corners = np.concatenate(
                    (self.lons_corners, self.lons_corners_add), axis=0
                )
                self.lats_corners = np.concatenate(
                    (self.lats_corners, self.lats_corners_add), axis=0
                )
        ...
        if self.model_type == "FV":
            self.lon_inds = np.where(
                (self.lon >= self.lon_range[0]) & (self.lon <= self.lon_range[-1])
            )[0]
            self.lat_inds = np.where(
                (self.lat >= self.lat_range[0]) & (self.lat <= self.lat_range[-1])
            )[0]
        ...
        if self.pretty_tick:
            if np.shape(self.colorticks) == ():
                if self.log_scale:
                    if self.cmin is None:
                        self.cmin_od = np.floor(
                            np.log10(
                                np.abs(np.min(self.var_slice[self.var_slice != 0]))
                            )
                        )
                    ...
                    if self.cmax is None:
                        self.cmax_od = np.floor(
                            np.log10(
                                np.abs(np.max(self.var_slice[self.var_slice != 0]))
                            )
                        )
                        self.cmax_sign = np.sign(
                            np.max(self.var_slice[self.var_slice != 0])
                        )
                    else:
                    ...
                    if self.sign_sum in [1, 2]:
                        ...
                        if nticks_init > 6:
                            ...
                            for nticks in Nticks_list:
                                if self.cmax_od > 3:
                                    ...
                                    tmparray = np.linspace(
                                        self.cmin_od, self.cmax_od, np.int(nticks)
                                    )
                                checksum = np.sum(
                                    np.abs(tmparray - tmparray.astype("i"))
                                )
                            ...
                            if check_loop:
                                ...
                                if self.cmax_od > 3:
                                    linticks = np.linspace(
                                        0, self.cmax_od, num=np.int(nticks)
                                    )
                                    self.colorticks = np.array(
                                        [0] + list(10 ** (linticks[1:]))
                                    )
                                else:
                                    self.colorticks = np.logspace(
                                        self.cmin_od, self.cmax_od, num=np.int(nticks)
                                    )
                        else:
                            nticks = nticks_init
                            linticks = np.linspace(
                        ...
                        if (self.cmin_od > 0) or (self.cmax_od > 0):
                            ...
                            nticks_init = (
                                self.cmax_od - self.cmin_od * self.cmin_sign + 1
                            )
                    ...
                    self.cbprop = get_cbar_prop(
                        [self.var_slice], Ntick_set=self.nticks, **self.kwd_pretty_tick
                    )
    ...
    def plot(self):
        ...
        if self.model_type == "FV":
            ...
            self.im = PolyCollection(
                self.verts, cmap=self.cmap, **self.kwd_polycollection
            )
        ...
        if self.lonlat_info:
            if self.lon_labels is None:
                self.lonticklabel = np.arange(
                    self.lon_range[0], self.lon_range[1] + 0.1, self.lon_interval
                )
            else:
            ...
            if self.lat_labels is None:
                self.latticklabel = np.arange(
                    self.lat_range[0], self.lat_range[1] + 0.1, self.lat_interval
                )
            else:
        ...
        if Ntick_set is None:
            ...
            for ntick in Nticks_list:
                ...
                if maxval < 3:
                    ...
                    checksum = np.sum(
                        np.abs(tmparray * factor - (tmparray * factor).astype("i"))
                    )
