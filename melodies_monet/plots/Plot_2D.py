...
class Plot_2D(object):
    def __init__(
        ...
        if type(var) in [xr.core.dataset.Dataset, xr.core.dataarray.DataArray]:
            if np.ndim(var) == 2:  # FV results
                ...
                if verbose:
                    print(
                        '"var" is a xarray variable, longitude and latitude values '
                        + "are being automatically assigned by xarray dimension variables"
                    )
                if lons is not None:
                    print('Warning: "lons" is assigned but not used ' + "because xarray itself has longitude values")
                if lats is not None:
                    print('Warning: "lats" is assigned but not used ' + "because xarray itself has longitude values")
        ...
        if self.model_type == "FV":  # 2D FV model output
            ...
            if (np.min(self.lon_range) < 0) & (np.max(self.corner_lon) > 180):
                if not self.center_180:
                    self.corner_lon[self.corner_lon > 180.0] -= 360.0
                    if verbose:
        ...
        if self.lonlat_info:
            if self.lon_interval is None:
                ...
                self.lon_interval = np.around(lon_length / 6.0)
                if self.lon_interval < 1:
                    self.lon_interval = 1
            if self.lat_interval is None:
                lat_length = lat_range[1] - lat_range[0]
                self.lat_interval = np.around(lat_length / 6.0)
                if self.lat_interval < 1:
                    self.lat_interval = 1

        # set vertices for SE model output
        if self.model_type == "SE":
            self.lons_corners = np.copy(self.corner_lon.reshape(self.corner_lon.shape[0], self.corner_lon.shape[1], 1))
            self.lats_corners = np.copy(self.corner_lat.reshape(self.corner_lat.shape[0], self.corner_lat.shape[1], 1))

            if not self.center_180:
                self.lons_corners[self.lons_corners > 180.0] -= 360
                self.center_lon[self.center_lon > 180.0] -= 360
                ...
                if self.center_180:
                    if lon_maxmin > 180:
                        if np.mean(self.lons_corners[i, :, :]) <= 180:
                            ...
                            inds = np.where(self.lons_corners[i, :, :] > 180)[0]
                            self.lons_corners[i, inds] = 360.0
                    ...
                    if lon_maxmin > 180:
                        if np.mean(self.lons_corners[i, :, :]) <= 0:
                            ...
                            tmp_lons_corners = np.copy(self.lons_corners[i, :])
                            tmp_lons_corners[inds2] = 180.0
                            self.lons_corners_add.append(tmp_lons_corners)
                            self.lats_corners_add.append(self.lats_corners[i, :])

                            inds = np.where(self.lons_corners[i, :, :] > 0)[0]
                            self.lons_corners[i, inds] = -180.0
            ...
            if self.center_180:
                self.lons_corners[self.lons_corners > 180.0] -= 360
                self.lons_corners_add[self.lons_corners_add > 180.0] -= 360
                self.center_lon[self.center_lon > 180.0] -= 360
            ...
            if self.lons_corners_add != []:
                self.lons_corners = np.concatenate((self.lons_corners, self.lons_corners_add), axis=0)
                self.lats_corners = np.concatenate((self.lats_corners, self.lats_corners_add), axis=0)
        ...
        if self.model_type == "FV":
            self.lon_inds = np.where((self.lon >= self.lon_range[0]) & (self.lon <= self.lon_range[-1]))[0]
            self.lat_inds = np.where((self.lat >= self.lat_range[0]) & (self.lat <= self.lat_range[-1]))[0]
            self.var_slice = self.var[self.lat_inds[0] : self.lat_inds[-1] + 1, self.lon_inds[0] : self.lon_inds[-1] + 1]
        elif self.model_type == "SE":
            self.ncol_inds = np.where(
                (self.center_lon >= self.lon_range[0])
                & (self.center_lon <= self.lon_range[-1])
                & (self.center_lat >= self.lat_range[0])
                & (self.center_lat <= self.lat_range[-1])
            )[0]
        ...
        if self.pretty_tick:
            if np.shape(self.colorticks) == ():
                if self.log_scale:
                    if self.cmin is None:
                        self.cmin_od = np.floor(np.log10(np.abs(np.min(self.var_slice[self.var_slice != 0]))))
                    ...
                    if self.cmax is None:
                        self.cmax_od = np.floor(np.log10(np.abs(np.max(self.var_slice[self.var_slice != 0]))))
                        self.cmax_sign = np.sign(np.max(self.var_slice[self.var_slice != 0]))
                    else:
                    ...
                    if self.sign_sum in [1, 2]:
                        ...
                        if nticks_init > 6:
                            ...
                            for nticks in Nticks_list:
                                if self.cmax_od > 3:
                                    tmparray = np.linspace(0, self.cmax_od, np.int(nticks))
                                else:
                                    tmparray = np.linspace(self.cmin_od, self.cmax_od, np.int(nticks))
                            ...
                            if check_loop:
                                ...
                                while tick_start > 0:
                                    ...
                                    tick_start -= interval
                                self.colorticks = np.array([0] + list(10 ** (np.flip(colorticks_h))))
                                nticks = len(self.colorticks)
                            else:
                                if self.cmax_od > 3:
                                    linticks = np.linspace(0, self.cmax_od, num=np.int(nticks))
                                    self.colorticks = np.array([0] + list(10 ** (linticks[1:])))
                                else:
                                    self.colorticks = np.logspace(self.cmin_od, self.cmax_od, num=np.int(nticks))
                        else:
                            nticks = nticks_init
                            linticks = np.linspace(self.cmin_od, self.cmax_od, np.int(nticks))
                            ...
                            if nticks_init > 6:
                                ...
                                for nticks in Nticks_list:
                                    if diff:
                                        ...
                                        tmparray = np.linspace(self.cmin_od * self.cmin_sign, self.cmax_od, np.int(nticks))
                                ...
                                if diff:
                                    if check_loop:
                                        ...
                                        while tick_start > 0:
                                            ...
                                            nticks += 1
                                        self.colorticks = (
                                            list(-(10 ** (np.array(colorticks_h)))) + [0] + list(10 ** (np.flip(colorticks_h)))
                                        )
                                        nticks = len(self.colorticks)
                                    else:
                                        linticks_hl = np.linspace(-self.cmin_od, 0, nticks)
                                        linticks_hr = np.linspace(0, self.cmax_od, nticks)[1:]
                                        self.colorticks = 10 ** (linticks_hl) * -1 + 10 ** (linticks_hr)
                                ...
                                nticks = nticks_init
                                linticks = np.linspace(self.cmin_od * self.cmin_sign, self.cmax_od, np.int(nticks))
                                zero_ind = np.where(linticks == 0)
                    ...
                    self.cbprop = get_cbar_prop([self.var_slice], Ntick_set=self.nticks, **self.kwd_pretty_tick)
            ...
            for ct in self.colorticks:
                if ct == 0:
                    ...
                    if format(np.abs(ct), ".4e")[tmpind - 2 : tmpind] == "00":
                        ...
                        lbtmp = sign + "$\\mathdefault{" + lbtmp.replace("e", "10^{")[:-2] + str(int(lbtmp[-2:])) + "}}$"
                    else:
                        lbtmp = format(ct, ".4e")[:tmpind] + format(ct, ".2e")[tmpind:]
                        lbtmp = "$\\mathdefault{" + lbtmp.replace("e", "\\times10^{")[:-2] + str(int(lbtmp[-2:])) + "}}$"
        ...
        if self.log_scale:
            ...
            try:
                self.kwd_pcolormesh["norm"] = matplotlib.colors.SymLogNorm(
                    linthresh=self.linthresh, linscale=self.linscale, vmin=self.cmin, vmax=self.cmax
                )
                self.kwd_polycollection["norm"] = matplotlib.colors.SymLogNorm(
                    linthresh=self.linthresh, linscale=self.linscale, vmin=self.cmin, vmax=self.cmax
                )
            except Exception:
                self.kwd_pcolormesh["norm"] = matplotlib.colors.SymLogNorm(
                    linthresh=self.linthresh, linscale=self.linscale, vmin=self.cmin, vmax=self.cmax, base=10
                )
                self.kwd_polycollection["norm"] = matplotlib.colors.SymLogNorm(
                    linthresh=self.linthresh, linscale=self.linscale, vmin=self.cmin, vmax=self.cmax, base=10
                )
    ...
    def plot(self):
        ...
        if self.model_type == "FV":
            ...
            self.im = PolyCollection(self.verts, cmap=self.cmap, **self.kwd_polycollection)
        ...
        if self.coast:
            self.ax.coastlines(resolution=self.resolution, lw=self.feature_line_lw, color=self.feature_color)
        if self.country:
            self.ax.add_feature(cfeature.BORDERS.with_scale(self.resolution), lw=self.feature_line_lw, edgecolor=self.feature_color)
        if self.state:
            self.ax.add_feature(cfeature.STATES.with_scale(self.resolution), lw=self.feature_line_lw, edgecolor=self.feature_color)
        if self.lonlat_info:
            if self.lon_labels is None:
                self.lonticklabel = np.arange(self.lon_range[0], self.lon_range[1] + 0.1, self.lon_interval)
            else:
                if self.lon_labels[0] > self.lon_labels[-1]:
                    self.lonticklabel = np.copy(self.lon_labels)
                    if self.center_180:
                        self.lonticklabel += 180
                        self.lonticklabel[self.lonticklabel > 180.0] -= 360

                else:
                    self.lonticklabel = self.lon_labels

            if self.lat_labels is None:
                self.latticklabel = np.arange(self.lat_range[0], self.lat_range[1] + 0.1, self.lat_interval)
            else:
            ...
            if self.regional:
                ...
                if self.lonlat_line:
                    for lontick in self.lonticklabel:
                        self.ax.plot([lontick, lontick], self.ax.get_ylim(), lw=1.0, linestyle=":", color="black", alpha=0.5)
                    for lattick in self.latticklabel:
                        self.ax.plot(self.ax.get_xlim(), [lattick, lattick], lw=1.0, linestyle=":", color="black", alpha=0.5)
        ...
        if self.title != "":
            self.ax.set_title(self.title, fontsize=self.title_size, fontfamily=self.font_family, **self.kwd_title)
...
class get_cbar_prop(object):
    def __init__(
        ...
        ranges=None,
        Nticks_list=None,
        max_find_method="ceil",
        tick_find_method="ceil",
        min_set=None,
        max_set=None,
        Ntick_set=None,
    ):
        ...
        if max_set:
            ...
            if maxval in ranges:
                ...
                plotmax = ranges[np.where(tmp >= 0.0)[0][-1]]
                # print( 'floor', plotmax )

            elif max_find_method == "ceil":
                tmp = maxval - np.array(ranges)
                plotmax = ranges[np.where(tmp >= 0.0)[0][-1] + 1]
                # print( 'ceil', plotmax )
