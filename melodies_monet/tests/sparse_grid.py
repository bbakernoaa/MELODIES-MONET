...
time_edges = np.linspace(
    start_timestamp, end_timestamp, ntime + 1, endpoint=True, dtype=float
)
...
time_da = xr.DataArray(
    time_grid, attrs={"longname": "time", "units": "seconds since 1970 Jan 01 00:00:00"}
)
...
count_grid_da = xr.DataArray(
    count_grid_array, dims=["time", "lat", "lon"], coords=[time_da, lat_da, lon_da]
)
data_grid_da = xr.DataArray(
    data_grid_array, dims=["time", "lat", "lon"], coords=[time_da, lat_da, lon_da]
)
