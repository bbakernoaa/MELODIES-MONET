...
for filename in files:
    ...
    grid_util.update_data_grid(
        time_edges, lat_edges, lon_edges, obs_ds["timestamps"], obs_ds["lat"], obs_ds["lon"], obs_ds[obs_var], count_grid, data_grid
    )

# normalize sparse data and convert to array data
grid_util.normalize_sparse_data_grid(count_grid_sparse, data_grid_sparse)
count_grid_array, data_grid_array = grid_util.sparse_data_to_array(
    time_edges, lat_edges, lon_edges, count_grid_sparse, data_grid_sparse
)
...
time_da = xr.DataArray(time_grid, attrs={"longname": "time", "units": "seconds since 1970 Jan 01 00:00:00"})
lat_da = xr.DataArray(lat_grid, attrs={"longname": "latitude", "units": "degree North"})
lon_da = xr.DataArray(lon_grid, attrs={"longname": "longitude", "units": "degree East"})

count_grid_da = xr.DataArray(count_grid_array, dims=["time", "lat", "lon"], coords=[time_da, lat_da, lon_da])
data_grid_da = xr.DataArray(data_grid_array, dims=["time", "lat", "lon"], coords=[time_da, lat_da, lon_da])
