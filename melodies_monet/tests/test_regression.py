...
def create_mock_data():
    ...
    mod_data.obj = xr.Dataset(
        ...
        coords={'time': [pd.Timestamp('2019-09-01')], 'lat': np.arange(10), 'lon': np.arange(10)}
    )
    ...
    obs_data.obj = xr.Dataset(
        ...
        coords={'time': [pd.Timestamp('2019-09-01')], 'latitude': (('site',), np.arange(5)), 'longitude': (('site',), np.arange(5))}
    )
...
def test_regression_sequential_vs_prefect():
    ...
    control_dict = {
        ...
        'evaluations': {
            'eval1': {'model': 'mod1', 'obs': 'obs1', 'mapping': {'O3': 'OZONE'}}
        }
    }
    ...
    with patch('melodies_monet.orchestrator.flows.main_orchestration_flow') as mock_flow:
