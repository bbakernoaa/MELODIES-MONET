...
def test_get_aqs_hourly(tmp_path):
    ...
    assert {v for v in ds.data_vars if ds[v].dims == ("time", "y", "x")} == {"OZONE", "time_local"}
...
def test_get_openaq(tmp_path):
    ...
    assert {v for v in ds.data_vars if ds[v].dims == ("time", "y", "x")} == {"o3", "pm25", "pm10", "time_local"}
...
def test_get_openaq_lowcost(tmp_path):
    ...
    assert ds.time.size == 1
    assert {v for v in ds.data_vars if ds[v].dims == ("time", "y", "x")} == {"pm25", "time_local"}
    assert (~ds.is_monitor).all()
