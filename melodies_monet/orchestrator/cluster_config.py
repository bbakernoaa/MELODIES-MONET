...
class ClusterFactory:
    ...
    @staticmethod
    def get_cluster(cluster_type=None, cluster_kwargs=None, adaptive_kwargs=None, platform=None, account=None, project=None):
        ...
        if platform and platform in PLATFORM_CONFIGS:
            ...
            if cluster_type is None: cluster_type = p_config["cluster_type"]
            for k, v in p_config.get("cluster_kwargs", {}).items():
                if k not in cluster_kwargs: cluster_kwargs[k] = v
            worker_groups = p_config.get("worker_groups", {}).copy()

        if cluster_type is None: cluster_type = "local"
        ...
        if cluster_type_lower == "local":
            ...
            if final_account and "account" not in cluster_kwargs: cluster_kwargs["account"] = final_account
            ...
            if final_account and "project" not in cluster_kwargs: cluster_kwargs["project"] = final_account
