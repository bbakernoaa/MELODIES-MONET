...
def mopitt_l3_pairing(model_data, obs_data, co_ppbv_varname, global_model=True):
    ...
    grid_adjust = xe.Regridder(
        model_obstime[["latitude", "longitude"]], obs_data[["lat", "lon"]], "bilinear", periodic=global_model, unmapped_to_nan=True
    )
...
def omps_l3_daily_o3_pairing(model_data, obs_data, ozone_ppbv_varname):
    ...
    grid_adjust = xe.Regridder(
        model_data[["latitude", "longitude"]], obs_data[["latitude", "longitude"]], "bilinear", periodic=True
    )
    ...
    for f in range(mod_nf):
        ...
        if len(tindex):
            ...
            regridr = xe.Regridder(
                model_data.isel(time=f), obs_data[["latitude", "longitude"]].sel(x=tindex), "bilinear"
...
def omps_nm_pairing(model_data, obs_data, ozone_ppbv_varname):
    ...
    ds = xr.Dataset(
        {ozone_ppbv_varname[0]: (["time", "y"], col), "ozone_column": (["time", "y"], obs_data.ozone_column.values)},
        coords={
...
def omps_nm_pairing_apriori(model_data, obs_data, ozone_ppbv_varname):
    ...
    ds = xr.Dataset(
        {ozone_ppbv_varname[0]: (["time", "y"], oz), "ozone_column": (["time", "y"], obs_data.ozone_column.values)},
        coords={
            "longitude": (["time", "y"], obs_data["longitude"].values),
            "latitude": (["time", "y"], obs_data["latitude"].values),
            "time": (["time"], obs_data.time.values),
