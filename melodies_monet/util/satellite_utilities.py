...
def mopitt_l3_pairing(model_data, obs_data, co_ppbv_varname, global_model=True):
    ...
    ## smooth/apply ak
    smoothed = obs_data["apriori_col"] + (obs_data["ak_col"] * diff_arr).sum(
        dim="alt", min_count=1
    )
    ...
    ds[co_ppbv_varname + "_column_model"] = ds[
        co_ppbv_varname + "_column_model"
    ].assign_attrs(units="$10^{18} molec./cm^{2}$")
    ds["column"] /= 1e18
...
def omps_l3_daily_o3_pairing(model_data, obs_data, ozone_ppbv_varname):
    ...
    column = (
        du_fac * (model_data["dp_pa"] / 100.0) * model_data[ozone_ppbv_varname]
    ).sum("z")
...
def space_and_time_pairing(model_data, obs_data, pair_variables):
    ...
    *** need to make setup work for surface/1z fields, as some pairing requires surface pressure field ***
    ...
    obs_nx, obs_ny = obs_data[
        "longitude"
    ].shape  # assumes 2d lat/lon fields in observation set
    ...
    for f in range(mod_nf):
        ...
        tindex = np.where(
            np.abs(obs_data.time - model_data.time[f])
            <= (model_data.time[1] - model_data.time[0])
        )[0]
            ...
            for j in pair_variables:
                ...
                if f == (mod_nf - 1):
                    ...
                    tind_2 = np.where(
                        ...
                        & (
                            np.abs(obs_data.time[tindex] - model_data.time[f])
                            <= (model_data.time[1] - model_data.time[0])
                        )
                    )[0]
                    tfac1 = 1 - (
                        np.abs(model_data.time[f] - obs_data.time[tindex[tind_2]])
                        / (model_data.time[1] - model_data.time[0])
                    )

                    ds[j][:, tindex[tind_2]] += (
                        np.expand_dims(tfac1.values, axis=1)
                        * interm_var[:, tind_2].values
                    )
                    ...
                    tfac1 = 1 - (
                        np.abs(model_data.time[f] - obs_data.time[tindex])
                        / (model_data.time[1] - model_data.time[0])
                    )

                    ds[j][:, tindex, :] += (
                        np.expand_dims(tfac1.values, axis=1) * interm_var.values
                    )
    return ds
...
def omps_nm_pairing(model_data, obs_data, ozone_ppbv_varname):
    ...
    du_fac = (
        1.0e-5 * 6.023e23 / 28.97 / 9.8 / 2.687e19
    )  # conversion factor; moves model from ppbv to dobson
...
def omps_nm_pairing_apriori(model_data, obs_data, ozone_ppbv_varname):
    ...
    for f in range(nf):
        ...
        if len(tindex):
            ...
            regrid_p = regridr(
                model_data["pres_pa_mid"][f]
            )  # this one should be pressure variable (for the interpolation).
            ...
            if f == (nf - 1):
                ...
                ozone_temp[:, tindex[tind_2], :] += (
                    np.expand_dims(tfac1.values, axis=1)
                    * regrid_oz[:, tind_2, :].values
                )
                pressure_temp[:, tindex[tind_2], :] += (
                    np.expand_dims(tfac1.values, axis=1) * regrid_p[:, tind_2, :].values
                )
                sfc[tindex[tind_2], :] += (
                    np.expand_dims(tfac1.values, axis=1) * sfp[tind_2, :].values
                )
            elif f == 0:
                t2 = np.where((obs_data.time[tindex] <= model_data.time[f]))[0]
                ozone_temp[:, tindex[t2], :] = regrid_oz[:, t2, :].values
                ...
                ozone_temp[:, tindex, :] += (
                    np.expand_dims(tfac1.values, axis=1) * regrid_oz.values
                )
                pressure_temp[:, tindex, :] += (
                    np.expand_dims(tfac1.values, axis=1) * regrid_p.values
                )
    ...
    ozone_satp = interplevel(
        ozone_temp[::-1], pressure_temp[::-1] / 100.0, obs_data.pressure, missing=np.nan
    )
