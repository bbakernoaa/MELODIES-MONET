...
def vertical_regrid(input_press, input_values, output_press):
    """
    Regrid vertical layers in a 3D array using linear interpolation.

    Parameters
    ----------
    input_press : np.ndarray
        Input pressure levels in hPa (lon, lat, alt).
    input_values : np.ndarray
        Input values to be regridded (lon, lat, alt).
    output_press : np.ndarray
        Output pressure levels in hPa (lon, lat, newalt).

    Returns
    -------
    np.ndarray
        The data regridded to the new pressure levels.
    """
...
def mod_to_overpasstime(modobj, opass_tms, partial_col=None):
    ...
    modobj : xr.Dataset
        Model data.
    opass_tms : pd.DatetimeIndex
        Satellite overpass local time.
    partial_col : str, optional
...
def check_timestep(model_data, obs_data):
    """
    Check and aggregate model data to match observation timestep for level 3 data.

    Assumes level 3 data has a monthly or daily timestep and that the model
    data has a higher or equal frequency.
...
def mopitt_l3_pairing(model_data, obs_data, co_ppbv_varname, global_model=True):
    ...
    model_data : xr.Dataset
        Model data.
    obs_data : xr.Dataset
        MOPITT observation data.
    co_ppbv_varname : str
        Variable name for CO in ppbv in the model.
    global_model : bool, optional
        Whether the model is global (used for periodic boundary in regridding).
        Default is True.

    Returns
    -------
    xr.Dataset
        Paired dataset with smoothed model CO column.
    """
    try:
        import xesmf as xe
    except ImportError:
    ...
    grid_adjust = xe.Regridder(
        model_obstime[["latitude", "longitude"]],
        obs_data[["lat", "lon"]],
        "bilinear",
        periodic=global_model,
        unmapped_to_nan=True,
    )
    co_model_regrid = grid_adjust(model_obstime[co_ppbv_varname])
    pressure_model_regrid = grid_adjust(model_obstime["pres_pa_mid"] / 100.0)
    ...
    for t in range(obs_data.time.size):
        ...
        co_regrid[t] = vertical_regrid(
            pressure_model_regrid.sel(time=obs_day).values.squeeze(),
...
def omps_l3_daily_o3_pairing(model_data, obs_data, ozone_ppbv_varname):
    ...
    Moves data from model grid to 1x1 degree OMPS L3 data grid. Following data
    grid matching, takes daily mean for model data.
    ...
    model_data : xr.Dataset
        Model data.
    obs_data : xr.Dataset
        OMPS L3 observation data.
    ozone_ppbv_varname : str
        Variable name for ozone in ppbv in the model.

    Returns
    -------
    xr.Dataset
        Paired dataset with model ozone column.
    """
    try:
        import xesmf as xe
    except ImportError:
    ...
    column = (du_fac * (model_data["dp_pa"] / 100.0) * model_data[ozone_ppbv_varname]).sum("z")

    # initialize regrid and apply to column data
    grid_adjust = xe.Regridder(
        model_data[["latitude", "longitude"]],
        obs_data[["latitude", "longitude"]],
        "bilinear",
        periodic=True,
    )
...
def space_and_time_pairing(model_data, obs_data, pair_variables):
    ...
    Assumes model data has (time, z, latitude, longitude) dimensions.
    Assumes observation data contains fields named time, latitude, and longitude.
    ...
    obs_data : xr.Dataset
        Observation data.
    pair_variables : list of str
        Variables to pair.

    Returns
    -------
    dict
        Dictionary containing paired model data arrays.
    """
    try:
        import xesmf as xe
    except ImportError:
        print("satellite_utilities: xesmf module not found")
        raise
    mod_nf, mod_nz, mod_nx, mod_ny = model_data[
        pair_variables[0]
    ].shape  # assumes model data is structured (time,z,lon,lat). lon/lat dimension order likely unimportant
    ...
    for f in range(mod_nf):
        ...
        if len(tindex):
            ...
            regridr = xe.Regridder(
                model_data.isel(time=f),
                obs_data[["latitude", "longitude"]].sel(x=tindex),
                "bilinear",
            )  # standard bilinear spatial regrid.
            ...
            for j in pair_variables:
                ...
                if f == (mod_nf - 1):
                    ...
                    tind_2 = np.where(
                        (obs_data.time[tindex] < model_data.time[f])
                        & (np.abs(obs_data.time[tindex] - model_data.time[f]) <= (model_data.time[1] - model_data.time[0]))
                    )[0]
                    tfac1 = 1 - (
                        np.abs(model_data.time[f] - obs_data.time[tindex[tind_2]]) / (model_data.time[1] - model_data.time[0])
                    )
...
def omps_nm_pairing(model_data, obs_data, ozone_ppbv_varname):
    """
    Pair model ozone mixing ratio with OMPS nadir mapper retrievals.

    Calculates column without applying apriori.
    ...
    model_data : xr.Dataset
        Model data.
    obs_data : xr.Dataset
        OMPS NM observation data.
    ozone_ppbv_varname : list of str
        List containing the variable name for ozone in ppbv in the model.
    ...
    col = np.nansum(
        du_fac * (paired_ds["dp_pa"] / 100.0) * paired_ds[ozone_ppbv_varname[0]], axis=0
    )  # new dimensions will be (satellite_x, satellite_y)
    ds = xr.Dataset(
        {
            ozone_ppbv_varname[0]: (["time", "y"], col),
...
def omps_nm_pairing_apriori(model_data, obs_data, ozone_ppbv_varname):
    """
    Pair model ozone mixing ratio data with OMPS nm, applying satellite apriori.

    Applies satellite apriori column to model observations.
    ...
    -------
    xr.Dataset
        Paired dataset with apriori-adjusted model ozone column.
    """
    try:
        import xesmf as xe
    except ImportError:
    ...
    ozone_satp = interplevel(ozone_temp[::-1], pressure_temp[::-1] / 100.0, obs_data.pressure, missing=np.nan)
    ...
    for i in range(nl):
        if i != 0:
            ...
            sfc[sfc == 0] = np.nan
            dp = np.abs(sfc / 100.0 - obs_data.pressure[i].values) + band[i]
    ...
    ds = xr.Dataset(
        {
            ozone_ppbv_varname[0]: (["time", "y"], oz),
            "ozone_column": (["time", "y"], obs_data.ozone_column.values),
        },
        coords={
            "longitude": (["time", "y"], obs_data["longitude"].values),
