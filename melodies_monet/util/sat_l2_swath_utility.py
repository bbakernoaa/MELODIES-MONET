...
def trp_interp_swatogrd(obsobj, modobj, no2varname="no2"):
    ...
    no2_modgrid_avg = xr.Dataset(
        data_vars={
            "nitrogendioxide_tropospheric_column": (["time", "x", "y"], np.full([nobstime, ny, nx], np.nan, dtype=np.float32)),
    ...
    for nd in range(nobstime):
        ...
        for ns in range(nswath):
            ...
            regridder = xe.Regridder(
                grid_in, no2_modgrid_avg[["lat", "lon"]], "bilinear", ignore_degenerate=True, reuse_weights=False
            )
