...
def trp_interp_swatogrd(obsobj, modobj, no2varname="no2"):
    ...
    nmodt, nz, ny, nx = modobj[
        f"{no2varname}_col"
    ].shape  # time, z, y, x, no2 columns at molec cm^-2
    ...
    for nd in range(nobstime):
        ...
        if "pres_pa_trop" in list(modobj.keys()):
            ...
            no2_modgrid_avg[f"{no2varname}trpcol"][nd, :, :] = (
                modobj_tm[f"{no2varname}_col"].sum(dim="z").values.squeeze()
            )
        ...
        for ns in range(nswath):
            ...
            try:
                ...
                raise ImportError(
                    "xesmf is required for swath regridding. Please install xesmf and esmpy."
                )
...
def trp_interp_swatogrd_ak(obsobj, modobj, no2varname="no2"):
    ...
    for nd in range(nobstime):
        ...
        for ns in range(nswath):
            ...
            mod_pres_no2 = modobj_tm[["pres_pa_mid", f"{no2varname}_col"]].mean(
                dim="time"
            )  # .transpose('y','x','z')
