...
def trp_interp_swatogrd(obsobj, modobj, no2varname="no2"):
    """
    Interpolate satellite swath to model grid.
    ...
    no2varname : str, optional
        Variable name for NO2. Default is 'no2'.

    Returns
    -------
    xr.Dataset
        Regridded satellite data at model grids for all datetimes.
    """
    ...
    for nd in range(nobstime):
        ...
        if "pres_pa_trop" in list(modobj.keys()):
            no2_modgrid_avg[f"{no2varname}trpcol"][nd, :, :] = (
                modobj_tm[f"{no2varname}_col"]
                .where(modobj_tm["pres_pa_mid"] >= modobj_tm["pres_pa_trop"])
                .sum(dim="z")
                .values.squeeze()
            )
        ...
        no2_modgrid_avg["nitrogendioxide_tropospheric_column"][nd, :, :] = np.nanmean(
            np.where(no2_modgrid_all > 0.0, no2_modgrid_all, np.nan), axis=2
        )
...
def trp_interp_swatogrd_ak(obsobj, modobj, no2varname="no2"):
    """
    Interpolate satellite swath to model grid with averaging kernel applied.
    ...
    for nd in range(nobstime):
        ...
        for ns in range(nswath):
            ...
            grid_sat = {
                "lon": working_swath["lon"].values,
                "lat": working_swath["lat"].values,
            }
            grid_mod = {
                "lon": modobj.coords["longitude"].values,
                "lat": modobj.coords["latitude"].values,
            }

            nysat, nxsat, nzsat = working_swath["averaging_kernel"].shape

            # regridding from model grid to sat grid
            regridder_ms = xe.Regridder(
                grid_mod,
                grid_sat,
                "bilinear",
                ignore_degenerate=True,
            ...
            working_swath["averaging_kernel"] = (
                working_swath["averaging_kernel"]
                * working_swath["air_mass_factor_total"]
                / working_swath["air_mass_factor_troposphere"]
            )
            # calculate the revised tamf_mod, and ratio = tamf_mod / tamf_org
            ratio = cal_amf_wrfchem(
                working_swath["averaging_kernel"],
                mod_rgd_sat["pres_pa_mid"].values,
                working_swath["preslev"],
                working_swath["troppres"],
                mod_rgd_sat[f"{no2varname}_col"].values,
                working_swath["air_mass_factor_troposphere"],
                grid_sat["lon"],
                grid_sat["lat"],
                grid_mod["lon"],
...
def cal_amf_wrfchem(
    ...
    """
    Calculate Air Mass Factor (AMF) for WRF-Chem using satellite averaging kernel.

    Parameters
    ----------
    scatw : xr.DataArray
        Scattering weights.
    wrfpreslayer : np.ndarray
        WRF pressure layers.
    tpreslev : np.ndarray
        Satellite pressure levels.
    troppres : np.ndarray
        Tropopause pressure.
    wrfno2layer_molec : np.ndarray
        WRF NO2 layers in molecules.
    tamf_org : np.ndarray
        Original tropospheric AMF.
    satlon : np.ndarray
    ...
    lb = np.where(
        (satlon >= np.nanmin(modlon))
        & (satlon <= np.nanmax(modlon))
        & (satlat >= np.nanmin(modlat))
        & (satlat <= np.nanmax(modlat))
    )
    ...
    for llb in range(len(lb[0])):
        ...
        f = interpolate.interp1d(
            np.log10(vertical_pres[:]), vertical_scatw[:], fill_value="extrapolate"
        )  # relationship between pressure to avk
    ...
    print(
        "Done with Averaging Kernel revision,",
        "factor min:",
        np.nanmin(ratio),
        "max:",
        np.nanmax(ratio),
