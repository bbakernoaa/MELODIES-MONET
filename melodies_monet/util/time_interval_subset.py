...
def subset_model_filelist(all_files, timeformat, timestep, timeinterval):
    ...
    timeformat : str
        The format of the time in the filename (e.g., '%Y%m%d').
    timestep : str
        The frequency of the time steps (e.g., '1h').
    timeinterval : list of pd.Timestamp
        A list containing [start_time, end_time].

    Returns
    -------
    list of str
        The subsetted list of file paths.
    """
    import pandas as pd
...
def subset_OMPS_l2(file_path, timeinterval):
    """
    Subset OMPS L2 filelist based on a specific naming convention.

    Convention: OMPS-satelliteid_NMTO3-L2_version_startingtimestamp_orbitnumber_endingtimestamp.h5
...
def subset_mopitt_l3(file_path, timeinterval):
    """
    Subset MOPITT L3 filelist based on naming conventions.
...
def subset_MODIS_l2(file_path, timeinterval):
    """
    Subset MODIS L2 filelist based on a specific naming convention.
