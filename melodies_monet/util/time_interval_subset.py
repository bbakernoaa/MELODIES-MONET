...
def subset_model_filelist(all_files, timeformat, timestep, timeinterval):
    ...
    subset_interval = pd.date_range(
        start=timeinterval[0], end=timeinterval[-1], freq=timestep
    )
    interval_files = []
...
def subset_OMPS_l2(file_path, timeinterval):
    ...
    interval_files = []
    subset_interval = pd.date_range(
        start=timeinterval[0], end=timeinterval[-1], freq="D", inclusive="left"
    )

    for i in subset_interval:
        fst = fnmatch.filter(
            all_files, "*OMPS-*_NMTO3-L2_v*_{}*_o*".format(i.strftime("%Ym%m%d"))
        )
        fst.sort()
        for j in fst:
...
def subset_mopitt_l3(file_path, timeinterval):
    ...
    if "MOP03JM-" in all_files[0]:
        subset_interval = pd.date_range(
            start=timeinterval[0], end=timeinterval[-1], freq="M"
        )
        strfmt = "%Y%m"
    else:
        subset_interval = pd.date_range(
            start=timeinterval[0], end=timeinterval[-1], freq="D"
        )
        strfmt = "%Y%m%d"
    for i in subset_interval:
...
def subset_MODIS_l2(file_path, timeinterval):
    ...
    interval_files = []
    subset_interval = pd.date_range(
        start=timeinterval[0], end=timeinterval[-1], freq="h", inclusive="left"
    )

    for i in subset_interval:
        fst = fnmatch.filter(
            all_files, "*M?D04_L2.A{}*.hdf".format(i.strftime("%Y%j.%H"))
        )
        fst.sort()
        for j in fst:
            interval_files.append(j)
