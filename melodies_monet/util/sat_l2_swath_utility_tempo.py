...
def regrid_and_apply_weights(
    ...
    if isinstance(obsobj, xr.Dataset):
        regridded = _regrid_and_apply_weights(obsobj, modobj, method=method, weights=weights, species=species, tempo_sp=tempo_sp)
    ...
    if len(ordered_keys) > 1:
        for k in ordered_keys[1:]:
            if paireddict[k].attrs["scan_num"] == scan_num:
                ...
                keys_in_scan = [k]
    regridded_scan = back_to_modgrid(paireddict, modobj, keys_in_scan, add_time=True, method=method, grid_path=grid_path)
    out_regridded = xr.merge([out_regridded, regridded_scan])
