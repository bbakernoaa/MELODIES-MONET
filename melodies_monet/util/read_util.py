            ...
            for group in filenames.keys():
                if isinstance(filenames[group], str):
                    files[group] = sorted(
                        [
                            file
                            for sublist in [glob(os.path.join(read_dir, file)) for file in [filenames[group]]]
                            for file in sublist
                        ]
                    )
                else:
                    if filenames[group][0].startswith("example:"):
                        files[group] = sorted(
                            [
                                file
                                for sublist in [
                                    [tutorial.fetch_example(":".join(s.strip() for s in file.split(":")[1:]))]
