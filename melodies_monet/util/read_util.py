    ...
    **kwargs : optional
    ...
    if method == "pkl":
        if isinstance(filenames, str):
            files = sorted(
                [
                    file
                    for sublist in [
                        glob(os.path.join(read_dir, file)) for file in [filenames]
                    ]
                    for file in sublist
                ]
            )
        ...
        if isinstance(filenames, dict):
                ...
                if isinstance(filenames[group], str):
                    files[group] = sorted(
                        [
                            file
                            for sublist in [
                                glob(os.path.join(read_dir, file))
                                for file in [filenames[group]]
                            ]
                            for file in sublist
                        ]
                    )
            ...
            raise TypeError(
                "NetCDF format filenames need to be specified as a dict, with format {group1:str or iterable of filenames, group2:...}"
            )
    ...
    if method == "pkl":
            ...
            for count, file in enumerate(files):
                if count == 0:
                    ...
                    for group in attr_out.keys():
                        attr_out[group].obj = xr.merge(
                            [attr_out[group].obj, attr_append[group].obj]
                        )
        ...
        setattr(
            analysis,
            attr,
            xarray_to_class(class_type=class_names[attr], group_ds=xr_dict),
        )
        ...
        for count, file in enumerate(filenames):
            ...
            if count == 0:
                ...
                if group_name1 != ds_append.attrs["group_name"]:
                    raise Exception(
                        "The group names are not consistent between the netcdf files being read."
                    )
                else:
