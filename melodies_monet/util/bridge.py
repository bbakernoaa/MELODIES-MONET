...
def compute_stats(paired_dict, output_dir='./', debug=False, **kwargs):
    ...
    **kwargs : Any
    ...
    for label, p_inst in paired_dict.items():
        ...
        from melodies_monet.stats.proc_stats import produce_stat_dict, create_table
        ...
        if kwargs.get('create_table', True):
            create_table(stat_df, outname=outname, title=f"Stats for {label}",
                         out_table_kwargs=kwargs.get('out_table_kwargs'), debug=debug)
        ...
        for i, modvar in enumerate(p_inst.model_vars):
            ...
            try:
                ...
                if plot_type == 'timeseries' and isinstance(data_to_plot, xr.Dataset):
                    ...
                    if 'col1' in sig_init.parameters: init_args['col1'] = obsvar
                    if 'col2' in sig_init.parameters: init_args['col2'] = modvar
