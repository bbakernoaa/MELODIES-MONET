...
def resample_stratify(da, levels, vertical, axis=1, interpolation="linear", extrapolation="nan"):
    ...
    result = stratify.interpolate(
        levels, vertical.chunk().data, da.chunk().data, axis=axis, interpolation=interpolation, extrapolation=extrapolation
    )
    dims = da.dims
...
def vert_interp(ds_model, df_obs, var_name_list):
    ...
    final_df_model = merge_asof(df_obs, df_model, by=["latitude", "longitude", "pressure_obs"], on="time", direction="nearest")
...
def mobile_and_ground_pair(ds_model, df_obs, var_name_list):
    ...
    final_df_model = merge_asof(
        df_obs, df_model, by=["latitude", "longitude"], on="time", direction="nearest", suffixes=("", "_new")
    )
